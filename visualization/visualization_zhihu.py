...
def generate_step_visualization(
    current_step: int, current_tokens: List[str], token_steps: List[int], max_step: int
) -> str:
    ...
    for idx, token in enumerate(current_tokens):
        ...
        if token == "<|mdm_mask|>":
            style.extend(
                ["color: transparent", "background: #f8fafc", "text-shadow: none"]
            )
            display_text = "&#8203;"
        else:
...
def main(target_step: int = 64):
    ...
    with open(
        f"{target_dir}/visualization_step_{target_step}.html", "w", encoding="utf-8"
    ) as f:
        f.write(f"""<html>
<head>
